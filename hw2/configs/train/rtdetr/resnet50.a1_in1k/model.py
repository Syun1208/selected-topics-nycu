...
# resnet50.a1_in1k out_indices=(2,3,4) → channels [512, 1024, 2048], strides [8, 16, 32]
_IN_CHANNELS = [512, 1024, 2048]
_FEAT_STRIDES = [8, 16, 32]
_FEAT_NAMES = ["p2", "p3", "p4"]
...
class AnchorPointGrid:
    """
    Anchor-free reference point generator.
    Produces a uniform (cx, cy) grid at each feature level — these are the
    "anchors" in anchor-free detection: points in image space, not boxes.
    The decoder predicts (dx, dy, dw, dh) offsets relative to each point.
    """

    def __init__(self, feat_strides: list, offset: float = 0.5):
        self.feat_strides = feat_strides
        self.offset = offset  # 0.5 → point at cell centre

    def __call__(self, feat_shapes: list, device) -> list:
        """
        Args:
            feat_shapes: [(H0,W0), (H1,W1), ...] per feature level
        Returns:
            list of (Hi*Wi, 2) float tensors with (cx, cy) in pixel coords
        """
        grids = []
        for (h, w), stride in zip(feat_shapes, self.feat_strides):
            ys = (torch.arange(h, device=device, dtype=torch.float32) + self.offset) * stride
            xs = (torch.arange(w, device=device, dtype=torch.float32) + self.offset) * stride
            gy, gx = torch.meshgrid(ys, xs, indexing="ij")
            grids.append(torch.stack([gx.flatten(), gy.flatten()], dim=1))
        return grids


class RTDETRDetectron2(nn.Module):
    """
    RT-DETRv2 with timm ResNet-50 (resnet50.a1_in1k) backbone.
    """
    ...
    def __init__(
        ...
        self.anchor_grid = AnchorPointGrid(feat_strides=_FEAT_STRIDES)
        ...
        feat_channels = [hidden_dim] * len(_IN_CHANNELS)
    ...
    def _apply_nms(self, result: dict) -> dict:
        """Score threshold + class-aware NMS on a single-image result dict."""
        ...
        keep = tvops.batched_nms(boxes, scores, labels, self.nms_iou_threshold)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            feat_dict = self.backbone(samples)
            feats = [feat_dict[k] for k in _FEAT_NAMES]
            # expose anchor grid for debugging / visualization
            self._last_anchor_grids = self.anchor_grid(
                [(f.shape[2], f.shape[3]) for f in feats], device=self.device
            )
            ...
            results = [self._apply_nms(r) for r in results]
...
dataloader.evaluator = L(COCOEvaluator)(
    dataset_name="valid",
    output_dir="logs/rtdetr/resnet50.a1_in1k/0",
)
...
train = dict(
    ...
    wandb=dict(
        ...
        params=dict(
            dir="./wandb_output",
            project="hw2_detrex",
            name="rtdetr_resnet50_a1_in1k_exp0",
        ),
    ),
    model_ema=dict(
        enabled=False,
