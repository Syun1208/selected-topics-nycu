...
from models.openglue.models.features import get_feature_extractor
...
def process_chunk(process_id: int, images_list: List[Tuple[str, Union[str, None]]], feature_extractor_config: dict,
    ...
    with torch.inference_mode():
        ...
        for i, (image_path, scene) in enumerate(images_list, start=1):
            ...
            resized_img, img, scale = feature_extractor.read_image(image_path)
            orig_height, orig_width, *_ = img.shape
            lafs, scores, descriptors = map(lambda x: x.cpu().numpy(), feature_extractor(resized_img, img, scale))

            # save results
            # NOTE:
            # Save originial sizes as resized sizes, 
            # because lafs have been already rescaled in feature_extractor.forward(),
            save_outputs(
                output_path_scene,
                base_name,
                (lafs, scores, descriptors, np.array([orig_width, orig_height]))
            )
