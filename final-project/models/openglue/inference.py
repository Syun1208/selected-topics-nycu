...
from models.openglue.models.laf_converter import get_laf_to_sideinfo_converter
from models.openglue.models.superglue.superglue import SuperGlue
from models.openglue.utils.misc import arange_like
...
@torch.no_grad()
def initialize_models(experiment_path: str,
                      checkpoint_name: str,
                      descriptor_dim: int,
    ...
    #config['inference']['match_threshold'] = 0.02
    ...
    # Add
    #descriptor_dim = config['features']['descriptor_dim']
    ...
    if match_threshold is not None:
        config['inference']['match_threshold'] = float(match_threshold)
    print(config)
...
class OpenGlueMatcher(nn.Module):
    ...
    def __init__(self, matcher: nn.Module, match_config: Optional[Dict] = None) -> None:
        super().__init__()
        self.match_config = match_config or {}
        self.laf_converter = get_laf_to_sideinfo_converter(self.match_config['superglue']['laf_to_sideinfo_method'])
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        b0, h0, w0 = (1, data['h0'], data['w0'])
        b1, h1, w1 = (1, data['h1'], data['w1'])
        ...
        mask0 = indices0 != -1
        _arange = torch.arange(1, device=descs0.device)
        batch_idxs0 = _arange.unsqueeze(1).expand(-1, indices0.shape[1]).long()
...
def prepare_features_output(lafs, responses, desc, laf_converter, permute_desc=False, log_response=False):
    ...
    kpts = lafs[:, :, :, -1]    # Shape(1, N, 2), xy order
...
def load_openglue_matcher(
    ...
    matcher, config = initialize_models(experiment_path,
                                        checkpoint_name,
                                        descriptor_dim,
                                        device=device,
                                        resize_to=resize_to,
                                        match_threshold=match_threshold)
    sg = OpenGlueMatcher(matcher, config)
    return sg


def match_og_superglue(
    matcher: OpenGlueMatcher,
    data: dict
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    with torch.no_grad():
        out = matcher(data)
    idxs = out['original_matching_idxs'].cpu().squeeze(0).numpy()
    matched_kpts0 = out['keypoints0'].cpu().squeeze(0).numpy()
    matched_kpts1 = out['keypoints1'].cpu().squeeze(0).numpy()

    # For OpenGlue bug (1 point case)
    if idxs.shape == (2,):
        idxs = idxs[None, :]
    if matched_kpts0.shape == (2,):
        matched_kpts0 = matched_kpts0[None, :]
    if matched_kpts1.shape == (2,):
        matched_kpts1 = matched_kpts1[None, :]

    if len(idxs) == 0:
        idxs = np.empty((0, 2), dtype=idxs.dtype)
    if len(matched_kpts0) == 0:
        matched_kpts0 = np.empty((0, 2), dtype=matched_kpts0.dtype)
    if len(matched_kpts1) == 0:
        matched_kpts1 = np.empty((0, 2), dtype=matched_kpts1.dtype)
    return idxs, matched_kpts0, matched_kpts1
