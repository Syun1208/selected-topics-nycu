# https://github.com/THU-LYJ-Lab/DarkFeat
import torch
...
class DarkFeat(nn.Module):
    ...
    def forward(self, input):
        ...
        return {
            'keypoints': kpts.to(data.device),
            'scores': torch.from_numpy(scores).to(data.device, non_blocking=True),
            'descriptors': torch.from_numpy(descs.T).to(data.device, non_blocking=True),
        }