...
import numpy as np
import cv2 as cv
from models.mtldesc.network import get_model, MTLDesc


class MTLDescCustomExtractor(object):
    ...
    def __init__(self,  **config):
        ...
        print(f'MTLDescExtractor.config: {self.config}')
    ...
    def _merge_and_generate_predict_point(
        self,
        batched_prob: np.ndarray,   # Shape(B, 1, H, W)
        height: int,
        width: int,
        kind: str = 'merge_keypoint'
    ):
        if kind == 'merge_keypoint':
            pts = []
            for i in range(len(batched_prob)):
                prob = batched_prob[i, 0]
                xs, ys = np.where(prob >= self.config['detection_threshold'])
                _pts = np.zeros((3, len(xs)))
                if len(xs) > 0:
                    _pts[0, :] = ys
                    _pts[1, :] = xs
                    _pts[2, :] = prob[xs, ys]
    ...
    def predict(self, imgs: List[np.ndarray], keys: str = "*"):
        self.model.eval()
        ...
        for img in resized_imgs:
            x = torch.from_numpy(img).to(torch.float).unsqueeze(dim=0).permute((0, 3, 1, 2)).to(self.device)
            x = (x / 255.) * 2. - 1.
            xs.append(x)
        x = torch.concat(xs)    # Shape(B, 3, H, W)

        # detector
        batched_heatmap, batched_feature, batched_weightmap = self.model(x)
        batched_prob = torch.sigmoid(batched_heatmap)
        batched_prob = batched_prob.detach().cpu().numpy()
        #prob = prob[0, 0]

        point, score = self._merge_and_generate_predict_point(
            batched_prob, height=scale_h, width=scale_w
        )  # [n,2]
        #weightmap=heatmap
        # descriptor
