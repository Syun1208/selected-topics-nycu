...
from transformers import AutoModelForMaskGeneration, AutoProcessor, pipeline, Pipeline
...
def segment(
    ...
    segmentator: Optional[nn.Module] = None,
...
def grounded_segmentation(
    ...
    segmenter_id: Optional[str] = None,
    segmentator: Optional[nn.Module] = None,
    segmentation_processor: Optional[Callable] = None,
) -> Tuple[np.ndarray, List[DetectionResult]]:
    if isinstance(image, str):
        image = load_image(image)

    detections = detect(
        image, labels, threshold, detector_id, object_detector=object_detector
    )
    detections = segment(
        image,
        detections,
