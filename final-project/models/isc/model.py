"""Adapted from https://github.com/lyakaap/ISC21-Descriptor-Track-1st/blob/master/isc_feature_extractor/model.py"""
...
def create_model(
...
) -> tuple[ISCNet, transforms.Compose, int, Any, Any]:
    """
    ...
    return (
        ...
        preprocessor,
        input_size,
        backbone.default_cfg["mean"],
        backbone.default_cfg["std"],
    )


def create_preprocessor(input_size: int, mean, std) -> transforms.Compose:
    preprocessor = transforms.Compose(
        [
            transforms.Resize((input_size, input_size)),
            transforms.ToTensor(),
