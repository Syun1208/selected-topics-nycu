"""Adapted from https://www.kaggle.com/code/fabiobellavia/imc2024-3d-metric-evaluation-example"""
...
from data import (
    DEFAULT_EVALUATION_ROOT_DIR,
    DEFAULT_SEED_VALUE,
)
from utils.debug import setup_data_and_pipeline
...
translation_thresholds_meters_dict = {
    ...
    #"church": np.array([0.025, 0.05, 0.1, 0.2, 0.5, 1.0, 20.0]),
    "lizard": np.array([0.025, 0.05, 0.1, 0.2, 0.5, 1.0]),
    "dioscuri": np.array([0.025, 0.05, 0.1, 0.2, 0.5, 1.0]),
}

PRESET_FILES = {
    "church": ["00057.png", "00055.png", "00070.png"]
}


def parse_args() -> Namespace:
    parser = ArgumentParser()
    parser.add_argument("-c", "--conf", required=True, help="Pipeline config")
    parser.add_argument("-s", "--scenes", nargs="*")
    parser.add_argument("-n", "--max-num-samples", default=None, type=int)
...
def compute_score(
    ...
    return float(np.array(results_per_dataset).mean()), list(
        zip(names, results_per_dataset)
    )


def reduce_samples(
    df: pd.DataFrame, max_num_samples: int = 110, random_state: int = DEFAULT_SEED_VALUE
) -> pd.DataFrame:
    dfs = []
    for _, idx in df.groupby("scene").groups.items():
        if len(idx) > max_num_samples:
            dfs.append(df.loc[idx].sample(n=max_num_samples, random_state=random_state))
        else:
...
def main(config_path: str,
         ...
         preset: bool = False):
    filenames = None
    if preset and scene_names:
        filenames = PRESET_FILES[scene_names[0]]
    data, pipeline = setup_data_and_pipeline(config_path, scene_names=scene_names, max_num_samples=max_num_samples, head=head, filenames=filenames)
    submission_df = pipeline.run(data.df, data, save_snapshot=True)
    solution_df = data.df

    submission_df = submission_df.rename(columns={"image_name": "image_path"})
    solution_df = solution_df.rename(columns={"image_name": "image_path"})

    score, all_results = compute_score(solution_df, submission_df)
    print("========")
    print(score)
    print("========")
    names, results = zip(*all_results)
    score_table = {name: result for name, result in zip(names, results)}
    ...
    for name in SCENE_NAMES:
        if name in score_table:
            headers.append(name)
            values.append(f"{float(score_table[name]):.3f}")
