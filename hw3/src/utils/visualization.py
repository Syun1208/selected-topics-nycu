...
def _make_run_title(cfg: Optional[dict]) -> str:
    if cfg is None:
        return ""
    model = cfg.get("model", {})
...
def _denorm(img_tensor: np.ndarray) -> np.ndarray:
    """(C,H,W) normalized float → (H,W,3) uint8 RGB"""
    img = img_tensor.transpose(1, 2, 0) * _STD + _MEAN
...
def plot_loss_curves(
    ...
    save_path: str,
    extra_metrics: Optional[Dict[str, List[float]]] = None,
    cfg: Optional[dict] = None,
) -> None:
    run_tag = _make_run_title(cfg)
...
def plot_all_losses(
    ...
    """Plot each loss component in its own subplot + val mAP."""
    if not loss_history:
        return

    all_keys = sorted({k for ep in loss_history for k in ep.keys() if k != "loss"})
    # put total loss first
    ...
    for idx, key in enumerate(plot_keys):
        ...
        ax.plot(epochs, vals, linewidth=2.0, color=_TAB_COLORS[idx % len(_TAB_COLORS)],
                marker="o", markersize=3)
    ...
    if val_map_history:
        ...
        ax.plot(range(1, len(val_map_history) + 1), val_map_history,
                linewidth=2.0, color=_TAB_COLORS[len(plot_keys) % len(_TAB_COLORS)],
                marker="s", markersize=4)
        ax.set_title(f"Val Mask AP\n{run_tag}", fontweight="bold")
        ax.set_xlabel("Epoch", fontweight="bold")
        ax.set_ylabel("Mask AP", fontweight="bold")
...
def plot_ap_per_class(
    ...
    ax.set_ylabel("Mask AP (IoU=0.50)", fontweight="bold")
    ax.set_ylim(0, 1.15)
...
def plot_pr_curve(
    precision_per_class: Dict[int, np.ndarray],
    recall_per_class: Dict[int, np.ndarray],
...
def plot_f1_recall_curve(
    f1_per_class: Dict[int, np.ndarray],
    recall_per_class: Dict[int, np.ndarray],
    ap_per_class: Dict[int, float],
    save_path: str,
    cfg: Optional[dict] = None,
    ...
    for cls in range(NUM_CLASSES):
        ...
        if len(f1) > 0:
            ...
            ax.scatter(r[best], f1[best], s=60, color=_TAB_COLORS[cls % len(_TAB_COLORS)],
...
# ──────────────────────────────────────────────
#  6. Prediction visualization (6 random samples)
# ──────────────────────────────────────────────

def visualize_predictions(
    images: List[np.ndarray],
    gt_masks_list: List[List[np.ndarray]],
    gt_labels_list: List[List[int]],
    ...
    fig, axes = plt.subplots(nrows, ncols,
                             figsize=(10, 4.5 * nrows), squeeze=False)
    ...
    for row, idx in enumerate(indices):
        ...
        p_scores = [pred_scores_list[idx][i] for i in keep]

        gt_vis   = _overlay_masks(img, gt_masks_list[idx], gt_labels_list[idx])
        pred_vis = _overlay_masks(img, p_masks, p_labels)
        ...
        axes[row][1].set_title(
            f"Pred #{idx}  ({len(p_masks)} dets, thr={score_thr:.2f})", fontsize=10)
        axes[row][1].axis("off")

    fig.legend(handles=legend_patches, loc="lower center",
               ncol=NUM_CLASSES, bbox_to_anchor=(0.5, 0.0),
               fontsize=9, framealpha=0.9)
    fig.suptitle(
        f"Predictions vs Ground Truth{epoch_str}\n{run_tag}",
...
def plot_iou_distribution(
    ...
    ax.set_xlabel("Mask IoU", fontweight="bold")
...
# ──────────────────────────────────────────────
#  8. save_all_charts — orchestrator
# ──────────────────────────────────────────────
...
def save_all_charts(chart_dir: str, metrics_data: dict, cfg: Optional[dict] = None) -> None:
    ...
    if "vis_data" in metrics_data:
        ...
        visualize_predictions(
            ...
            str(d / "predictions_vis.png"),
            cfg=cfg,
            epoch=epoch,
        )

    if "iou_distribution" in metrics_data:
        plot_iou_distribution(
            metrics_data["iou_distribution"], str(d / "iou_distribution.png"), cfg=cfg)
