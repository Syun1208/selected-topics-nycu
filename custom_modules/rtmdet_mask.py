...
class RTMDetWithMaskHead(RTMDet):
    ...
    def loss(self, batch_inputs, batch_data_samples):
        ...
        max_per_img = self.train_cfg.get('max_mask_instances_per_img', None)
        gt_masks = []
        gt_bboxes = []
        for img_gt_instances in batch_gt_instances:
            ...
            n = img_bboxes.size(0)
            if max_per_img is not None and n > max_per_img:
                idx = torch.randperm(n, device=img_bboxes.device)[:max_per_img]
                img_masks = img_masks[idx]
                img_bboxes = img_bboxes[idx]
            gt_masks.append(img_masks)
    ...
    def predict(self, batch_inputs, batch_data_samples, rescale=True):
        ...
        for img_idx, (results,
            ...
            chunk_size = 128
            chunks = []
            for i in range(0, len(pred_bboxes), chunk_size):
                chunk_pred = self.mask_head(mask_feats[i:i + chunk_size])
                chunk = _do_paste_mask(
                    chunk_pred,
                    pred_bboxes[i:i + chunk_size],
                    img_h, img_w, False)[0] > 0
                chunks.append(chunk.cpu())
                del chunk_pred
            img_mask = torch.cat(chunks, dim=0) if chunks else torch.zeros(
                (0, img_h, img_w), dtype=torch.bool)
