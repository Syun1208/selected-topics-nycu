...
class MultiEMAOptimWrapperConstructor(DefaultOptimWrapperConstructor):
    """Optimizer constructor that skips frozen EMA submodules.

    EMA parameters have requires_grad=False and are updated via momentum, not
    gradients. Without this constructor, mmengine emits a WARNING for every
    single EMA parameter (~hundreds per run), flooding the log.
    """

    def add_params(self, params, module, prefix='', is_dcn_module=None):
        if prefix == 'ema_models' or prefix.startswith('ema_models.'):
            return
        super().add_params(params, module, prefix=prefix,
