...
class RandomRotateScaleCrop(BaseTransform):
    ...
    def transform(self, results):
        ...
        out_h, out_w = rot_img.shape[:2]
        if gt_masks.shape[0] == 0 or gt_masks.shape[1] == 0 or gt_masks.shape[2] == 0:
            rot_gt_masks = np.empty((0, out_h, out_w), dtype=gt_masks.dtype)
        else:
            # warpAffine only supports ≤4 channels; iterate over each mask
            # (H, W) individually to avoid the channel-count limit.
            rot_gt_masks = np.stack([
                cv2.warpAffine(
                    mask, M, self.img_scale,
                    flags=cv2.INTER_LINEAR,
                    borderMode=cv2.BORDER_CONSTANT,
                    borderValue=0,
                )
