...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, bias):
        ...
        self.qkv_dwconv = nn.Conv2d(
            dim * 3, dim * 3,
            kernel_size=3, stride=1, padding=1, groups=dim * 3, bias=bias,
        )
...
def build_promptir(model_cfg: dict) -> PromptIR:
    return PromptIR(
        inp_channels=model_cfg.get("inp_channels", 3),
        out_channels=model_cfg.get("out_channels", 3),
        dim=model_cfg.get("dim", 48),
        num_blocks=tuple(model_cfg.get("num_blocks", (4, 6, 6, 8))),
        num_refinement_blocks=model_cfg.get("num_refinement_blocks", 4),
        ...
        LayerNorm_type=model_cfg.get("layer_norm_type", "WithBias"),
        decoder=model_cfg.get("decoder", True),
    )
